...
#                add negative reward if target can see tracker
                Value_init[j] -= (1.0) * prob_target[i]
            if check(tracker_states[j], target_states[i], tracker, target):
#                add positive reward if tracker can see target
                Value_init[j] += (1.0)
...
# updating initial value function matrix with actions
    ...
    for i in range(len(tracker_states)):
        for ob in tracker.obstacles:
            x,y,a = tracker_states[i]
            min,max = ob
#            check if state is within obstacle
            if min[0] <= x <= max[0] and min[1] <= y <= max[1]:
                Value_init[i] = -10
#            check if state is out of bound
